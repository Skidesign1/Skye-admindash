...
const PartnerBlockToggle = ({ partnerId, isBlockedInitially = false }) => {
    const [isBlocked, setIsBlocked] = useState(isBlockedInitially); // ✅ Use default value if undefined

    const handleToggleBlock = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/admin/block-partner/${partnerId}`, {
                method: "PUT",
                headers: {
...
export default PartnerBlockToggle;
