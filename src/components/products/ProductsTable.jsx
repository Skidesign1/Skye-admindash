...
const ProductsTable = () => {
	...
	const deleteCreative = async (creativeId, e) => {
		...
		try {
			...
			setCreatives((prev) => prev.filter((c) => c._id !== creativeId));
		} catch (error) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const res = await fetch("http://localhost:5000/api/admin/creatives");
	...
	return (
		...
		>
			<div className='flex justify-between items-center mb-6'>
				<h2 className='text-xl font-semibold text-gray-100'>Creative List</h2>
				...
				<table className='min-w-full divide-y divide-gray-700'>
					<thead>
						<tr>
							...
							<th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider'>Templates</th>
